 ...
 * replacement) and the dynamic runtime-context snapshot. Copied from
 * `@deepseek-ai/dsh-agent-loop` (not exported there) so the pi-driven driver
 * keeps context-snapshot plugins such as time-context working unchanged.
 * @module @deepseek-ai/dsh-pi-agent-loop/runtime-context
 */
